...
xxx_AutoCompleteProvider.createSuggestion = function(item,completeFunArgs) {
	...
	var _g = item.k;
	if(_g == null) {
		console.log("src/xxx/AutoCompleteProvider.hx:296:","TODO");
		sug.text = item.t;
	} else {
		switch(_g) {
		case "literal":
...
xxx_AutoCompleteProvider.prototype = {
	...
	,getSuggestions: function(req) {
		...
		return new Promise(function(resolve,reject) {
			...
			console.log("src/xxx/AutoCompleteProvider.hx:71:","PREFIX = [" + prefix + "][" + replacementPrefix + "]");
...
xxx_Build.prototype = $extend(atom_Emitter.prototype,{
	start: function(verbose) {
		...
		window.console.group("%c" + "haxe " + this.args.join(" "),"color:" + "#F1471D" + ";");
		xxx_IDE.server.query(this.args,null,function(res) {
			window.console.groupEnd();
			console.log("src/xxx/Build.hx:57:","END");
			_gthis.emit("end",0);
		},function(err) {
	...
	,query: function(pos,mode,extraArgs) {
		...
		return new Promise(function(resolve,reject) {
			...
			xxx_IDE.server.query(args,preText,function(r) {
				...
				console.log("src/xxx/CompletionService.hx:134:",xml);
...
var xxx_IDE = function() { };
xxx_IDE.__name__ = true;
xxx_IDE.activate = function(state) {
	console.log("src/xxx/IDE.hx:34:","Atom-xxx");
	...
	haxe_Timer.delay(xxx_IDE.startServer,atom.config.get("xxx." + "haxe_server_startdelay") * 1000 | 0);
	...
	xxx_IDE.disposables.add(atom.project.onDidChangePaths(function(paths) {
		...
		xxx_IDE.projectPaths = paths;
		xxx_IDE.searchHxmlFiles(added,null,function(found) {
			console.log("src/xxx/IDE.hx:131:",found.length + " new hxml files found");
			xxx_IDE.hxmlFiles = xxx_IDE.hxmlFiles.concat(found);
		});
		return;
	}));
	...
	xxx_IDE.searchHxmlFiles(null,null,function(found1) {
		console.log("src/xxx/IDE.hx:149:",found1.length + " hxml files found");
		xxx_IDE.hxmlFiles = found1;
		if(state != null && state.hxml != null) {
			if(xxx_IDE.hxmlFiles.indexOf(state.hxml) != -1) {
				xxx_IDE.selectHxml(state.hxml);
			}
		} else {
			xxx_IDE.selectHxml(found1[0]);
		}
	});
};
...
xxx_IDE.consumeStatusBar = function(bar) {
	bar.addLeftTile({ item : xxx_IDE.statusbar.element, priority : -1000});
};
...
xxx_view_BuildView.openPosition = function(path,line,column,activatePane,searchAllPanes,callback) {
	...
	atom.workspace.open(path,{ initialLine : line, initialColumn : column, activatePane : activatePane, searchAllPanes : searchAllPanes, pending : false, split : null, activateItem : true}).then(function(editor) {
		if(callback != null) {
			callback(editor);
		}
	});
...
xxx_view_BuildView.openErrorPosition = function(err) {
	var line = err.line - 1;
	console.log("src/xxx/view/BuildView.hx:228:","openErrorPosition");
	console.log("src/xxx/view/BuildView.hx:229:",err);
